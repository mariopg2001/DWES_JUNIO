...
<?php
    ...
    echo '<form action="indexv4.php" method="post" >
...
<?php
    if(isset($_POST['enviar'])){
        ...
        if($resultado2->num_rows>0){ //si nos devuelve 0 filas el id de la visita no existe y si existe nos mostrará el lugar
            echo'<form method="post" action="indexv4.php">
                
                <label>Elige el lugar de visita</label>';
                //<select name="lugares">';

                $fila2=$resultado2-> fetch_assoc();//guardamos como array 
                $iplugar=$fila2['ip']; //lo añadimos a una variable para poder usarla fuera del while
                ...
                while($fila=$resultado4-> fetch_assoc() && $fila['ip']!=$iplugar)
                {
                  //if($fila['ip']==$iplugar){
                        echo 'avanza punteroc'.$fila['lugar'].' <br/>'; //añadimos el lugar seleccionado en la 1 opcion
                        //break;
                    //}
                }
