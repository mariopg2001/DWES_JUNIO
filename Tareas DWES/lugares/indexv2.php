...
<?php
    ...
    echo '<form action="indexv2.php" method="post" >
    ...
    if(isset($_POST['enviar'])){
        ...
        if($resultado2->num_rows>0){ //si nos devuelve 0 filas el id de la visita no existe y si existe nos mostrará el lugar
            echo'<form method="post" action="indexv2.php">
                ...
                {
                  if($fila['ip']==$iplugar){
                        echo '<option value="'.$iplugar.'">'.$fila['lugar'].'</option>';
                        break;
                    }
                }
                $resultado4->data_seek(0);//ponemos el indice en la posicion 0
                while($fila=$resultado4-> fetch_assoc())
                {
                    if($fila['ip']!=$iplugar){
                        echo "<option value=".$fila['ip'].">".$fila['lugar']."</option>"; //lo añadimos al option 
                    } 
                }
